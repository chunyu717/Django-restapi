...
專案內應用程式即是 "language".  此專案透過 djangorestframework 建立 api 包含不 template html 網頁的部分。
網頁部分實作可以參考 django-auth-practice (Login Page implement)
另外設定虛擬環境這個部分也參考 django-auth-practice 較好. 
...
##### 加入 api_example\api_example\urls.py  [ 將請求發送到正確的視圖(urls.py) ]
```
urlpatterns = [
...
![Screenshot](adminPage1.jpg)
![Screenshot](adminPage2.jpg)
...
# 編輯 laguages\views.py  [處理請求(views.py)]
```
